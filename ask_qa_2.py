import openai
from neo4j import GraphDatabase
from openai import OpenAI

# Neo4j Connection Setup
uri = "neo4j+s://b8d08542.databases.neo4j.io"  # Replace with your URI
username = "neo4j"              # Replace with your username
password = "6aGLLE1wrWr4pGornnwPnhHys6MU2Ag4pSO8SeFSc2I"           # Replace with your password
api_key = "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"
neo4j_driver = GraphDatabase.driver(uri, auth=(username, password))


# Function to Execute Cypher Query
def execute_cypher_query(cypher_query):
    with neo4j_driver.session() as session:
        results = session.run(cypher_query)
        return [record for record in results]

# Function to Format Neo4j Results
def format_neo4j_results(results):
    return '\n'.join([str(result) for result in results])

# Chat with OpenAI Function
def chat_with_openai(api_key, initial_message=None):
    client = OpenAI(api_key=api_key)
    messages = []

    if initial_message:
        messages.append({"role": "user", "content": initial_message})

    while True:
        user_input = input("User: ")
        messages.append({"role": "user", "content": user_input})

        # Send the query to GPT-3.5 and get the response
        full_response = ""
        for response in client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=messages,
                stream=True,
        ):
            full_response += (response.choices[0].delta.content or "")

        # Analyze response to decide if it's a Cypher query
        if "Cypher query:" in full_response:
            # Extract Cypher query from the response
            cypher_query = full_response.split("Cypher query:")[1].strip()
            try:
                neo4j_results = execute_cypher_query(cypher_query)
                formatted_results = format_neo4j_results(neo4j_results)
                print("Assistant:", formatted_results)
            except Exception as e:
                print("Error in executing Cypher query:", e)
        else:
            print("Assistant:", full_response)

        messages.append({"role": "assistant", "content": full_response})

if __name__ == "__main__":
    chat_with_openai(api_key)